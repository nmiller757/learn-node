...
app.use(cookieParser('12345-67890-09876-54321')); // secret key, signed key,  if client send cookie, its parsered and made availible in the req

function auth (req, res, next) {
    console.log(req.headers);

    if (!req.signedCookies.user) {// if cookie not in the header. 
	    ...
	    if (user == 'admin' && pass == 'password') {
	    	res.cookie('user','admin',{signed: true});//cookie is set name, value, and signed true
